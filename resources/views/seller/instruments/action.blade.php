<form id="delete-form-{{ $instrument->id }}" action="{{ route('instruments.destroy', $instrument->id) }}" method="post">
    @csrf
    @method('delete')
    ...
    <a href="#" class="btn btn-warning btn-sm" onclick="edit(this)" data-toggle="modal"
        data-target="#edit-instrument" data-manage_instrument_images="{{ route('images.show', $instrument->id) }}" data-name_instrument="{{ $instrument->name_instrument }}"
        ...
        data-url="{{ route('instruments.update', $instrument->id) }}">
