<form id="delete-form-{{ $slide->id }}" action="{{ route('slides.destroy', $slide->id) }}" method="post">
    @csrf
    @method('delete')
    ...
    <a href="#" class="btn btn-warning btn-sm" onclick="edit(this)" data-toggle="modal" data-target="#edit-slide"
        data-order="{{ $slide->order }}" data-url_image="{{ $slide->url_image }}" data-url="{{ route('slides.update', $slide->id) }}">
